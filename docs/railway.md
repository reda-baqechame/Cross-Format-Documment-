...
**Deployed-revision reporting.** `/api/health` and `/api/ready` expose `deployed_revision` so release automation (the post-merge `production-smoke` CI job) can prove it is testing the freshly-merged commit rather than a healthy-but-stale container. The revision resolves from `RAILWAY_GIT_COMMIT_SHA` → `SOURCE_COMMIT` → `GIT_COMMIT_SHA` (`settings.deployed_revision`). The root [`Dockerfile`](../Dockerfile) bakes the commit into the image at build time via the `RAILWAY_GIT_COMMIT_SHA` build arg (persisted as `SOURCE_COMMIT`), so the running container reports its revision even if Railway's runtime variable injection is unavailable. If `deployed_revision` reports `"unknown"` in production, Railway is not exposing `RAILWAY_GIT_COMMIT_SHA` as a build arg for this service — confirm the service is GitHub-connected (provided variables are only injected for git-triggered deploys).
