...
2. **Document engine** (`services/docengine`) — a registry of `FormatAdapter`s. Each
   ...
   ways against the canonical model. The registry ships functional adapters for TXT, MD,
   HTML, RTF, DOCX, PDF, XLSX, PPTX, CSV, JSON, XML, PNG, JPEG, TIFF, and more — see
   ``docs/implementation-status.md`` for fidelity notes and AGPL considerations on PDF.
