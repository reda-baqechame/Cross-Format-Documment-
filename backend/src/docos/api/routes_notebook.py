...
async def notebook_ask(
    body: NotebookRequest, session: Session = Depends(db_session)
) -> NotebookResponse:
    question = body.question.strip()
    if not question:
        raise HTTPException(status_code=422, detail="question is required")

    docs = load_corpus(session, body.doc_ids)
    use_llm = get_settings().llm_provider != "noop"
    result = await corpus_service.notebook_answer(
        docs, question, get_llm_client(), use_llm=use_llm
    )
