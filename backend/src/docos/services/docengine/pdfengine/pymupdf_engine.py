"""PyMuPDF (fitz) PdfEngine — the current default implementation, isolated behind the boundary.

This is the existing behaviour, unchanged: it moves the fitz call sites that lived in
``pageops.py`` verbatim into this module so the rest of the codebase stops importing ``fitz``
directly. fitz is AGPL-3.0; this module is the *only* place structural PDF operations touch it,
which is what makes the permissive migration auditable.

Text/table extraction, redaction write-back, and searchable-PDF writing are intentionally NOT
here yet — those still import fitz in their own modules (see the parity gate). They will move
behind this boundary once their permissive replacements reach fidelity parity.
"""
